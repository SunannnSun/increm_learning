...
tic;
[Priors, Mu, Sigma, assignment_array] = fit_gmm(Xi_ref, Xi_dot_ref, est_options);
toc;

figure;

color_array = [];
for k=1:max(assignment_array)
    color_array= [color_array; rand(1,3)];
end
for i=1:size(assignment_array, 2)
    scatter(Xi_ref(1, i), Xi_ref(2, i),'filled', "MarkerFaceColor",color_array(assignment_array(i), :))
    hold on
end
